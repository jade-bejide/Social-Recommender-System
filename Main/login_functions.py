...
def showRatingsWindow(window, user, title):
    global system

    ratingWindow = Window(window, title="Rating Movies")
    ...
    star_1 = PushButton(rateBox, text="1", command=lambda:system.saveRating(user, title, 1, ratingWindow))
    star_2 = PushButton(rateBox, text="2", command=lambda:system.saveRating(user, title, 2, ratingWindow))
    star_3 = PushButton(rateBox, text="3", command=lambda:system.saveRating(user, title, 3, ratingWindow))
    star_4 = PushButton(rateBox, text="4", command=lambda:system.saveRating(user, title, 4, ratingWindow))
    star_5 = PushButton(rateBox, text="5", command=lambda:system.saveRating(user, title, 5, ratingWindow))
...
def initialiseRatingsWindow(window, user, rateCapacity):
    ratings_queue = setUpQueue(rateCapacity)
    print(ratings_queue.getDataset())

    if ratings_queue.isEmpty() == False:
        title = ratings_queue.dequeue()
        print(title)
        showRatingsWindow(window, user, title)
